...
******CSV FILE NAMES ****
*HHQ CSV File name 
global HHQcsv RJR3_Household_Questionnaire_v6
*FQ CSV File name
global FQcsv RJR3_Female_Questionnaire_v6
...
local HHQdofile CCRX_HHQ_Datachecking_v13.0_1May2017_SJ

*FRQ_DataChecking File Name
local FRQdofile CCRX_FRQ_DataChecking_v19.0_1Aug2017_LZ

*HHQmember_DataChecking File Name
local HHQmemberdofile CCRX_HHQmember_DataChecking_v5.0_25Aug2016

*WASH do file
local WASHdofile CCRX_WASH_v13.0_17Jul2017_nge

*CleaningByRE_Female date and initials
local CleanFemaledate 2017Jun30_NS

*CleaningByRE_HHQ date and intitials
local CleanHHQdate 2017Jun30_NS

*Country Specific Clean Weight and initials
local CountrySpecificdate v22.1_2017Jun30_NS

*Country/Round specific module
local module1do CCRX_FRQchild_v6_Diarrhea_15Jun2017_HC
/*local module2do 
local module3do 
local module4do
*/
...
global csvdir "~/Dropbox (Gates Institute)/Rajasthan/PMADataManagement_Rajasthan/Round3/Data/CSV_Files"
*global csvdir "E:\Dropbox (Gates Institute)\PMADataManagement_Rajasthan\Round3\Data\CSV_Files"
*global csvdir "C:\Users\Shulin\Dropbox (Gates Institute)\PMADataManagement_Rajasthan\Round3\Data\CSV_Files"
*global csvdir  "/Users/jnobili/Dropbox (Gates Institute)/PMADataManagement_Rajasthan/Round3/Data/CSV_Files"

**Create a global data directory - 
global datadir "~/Dropbox (Gates Institute)/Rajasthan/Data_NotShared/Round3/Data/HHQFQ"
*global datadir "E:\Dropbox (Gates Institute)\PMA2020\Round 3\Data\HHQFQ"
*global datadir "C:\Users\Shulin\Dropbox (Gates Institute)\Monitor RJR3\9Oct2017"
*global datadir "/Users/jnobili/desktop/TEST"

**Create a global do file directory
global dofiledir "~/Dropbox (Gates Institute)/Rajasthan/PMADataManagement_Rajasthan/Round3/Cleaning_DoFiles/Current"
*global dofiledir "E:\Dropbox (Gates Institute)\PMADataManagement_Rajasthan\Round3\Cleaning_DoFiles\Current"
*global dofiledir "C:\Users\Shulin\Dropbox (Gates Institute)\PMADataManagement_Rajasthan\Round3\Cleaning_DoFiles\Current"
*global dofiledir  "/Users/jnobili/Dropbox (Gates Institute)/PMADataManagement_Rajasthan/Round3/Cleaning_DoFiles/Current"

//REVISION SJ 17AUG2017
**Create a global for analytics csv files
global analyticsdir "C:\Users\Shulin\Dropbox (Gates Institute)\PMADataManagement_Rajasthan\Round3\Data\Analytics"
*global analyticsdir "E:\Dropbox (Gates Institute)\PMADataManagement_Rajasthan\Round3\Data\Analytics"
...
*Windows command.  Erases all old files that start with `CCRX'
...
*REVISION - v20 - lz - 09May2017 added .do
...
start end today aquainted-firstname marital_status Latitude-Accuracy  {
rename `var' FQ`var'
}
...
metainstanceName FQfirstname FQ_age FQSubmissionDate FRS_result FQstart FQend unlinkedEA  using `CCRX'_HHQFQErrors_$date.xls, firstrow(variables) sh(FRS_Duplicate_in_Female) sheetreplace
	...
	if _rc==198 { 
		...
		*REVISION v21.0 LZ revising duplicate link sheet so that once duplicate links are cleaned they do not 
		*show up on Duplicate_Link_in_FRQ sheet
drop duplink
...
*REVISION-v19-LZ-2017.03.31 link was blank for unlinked forms and being dropped as duplicate
...
**REVISION -v19 -LZ- 2017.03.31 link issue
...
**REVISION-v19-LZ-2017.03.31 export out link variable into FRQmergeerror tab
...
collapse (sum) eligible if HHQ_result==1, by(RE $GeoID)
	...
	*generate Avrgeligible=totaleligible/HQcomplete
	*label var Avrgeligible	"Average number of eligible women per HH - COMPL`CCRX'ED FORMS ONLY"
	*drop R_*
	save, replace
...
forval y=13/15{
capture confirm water_uses_`x'
	if _rc==0{
	replace bottletag=1 if (regexm(water_uses_`x', "cooking")) | (regexm(water_uses_`x',"washing")) 
}
}	
	...
	preserve
		...
		tab bottletag
		sort RE
		capture noisily export excel metainstanceID RE GeoID_SH water_sources_all water_uses_13 water_uses_14 using `CCRX'_HHQFQErrors_$date.xls, firstrow(variables) sh(Bottledwater) sheetreplace
di _rc
if _rc!=198{
restore
}
...
gen datetag=1 if system_date_3!="2017" & system_date_3!="17"
drop system_date_*
...
replace datetag=1 if start_3!="2017" & start_3!="17"
drop start_*
...
replace datetag=1 if end_3!="2017" & end_3!="17"
drop end_*
...
gen DNKpenultbirth=1 if regexm(penultimate_birth, "2020")
...
*REVISION - v20 - lz - 09May2017 added 4th module
...
/*****Version History******
September 3, 2015 Adapted system_date, end, and start to allow for dates being 
inputted in two different formats
1 Dec 2015 fixed typon on line 305 changing unlinkede to unlinkedEA
2 Dec 2015 added macros for country and round, added these macros to HHQ do file and removed from country specific do file
14 Dec 2015 Adapted to include wash specific do file to allow for country specific programming
06 Feb 2016	Fixed shell erase command for windows so that old datasets are archived and then erased
	Also sort by FRS_form_name in FRS_Duplicate tab.  Changed name of DuplicateFRS to FRS_Duplicate_in_Female
	Changed DuplicateHouseholdInfo tab to DuplicateHH_DifferentNames
08 Feb 2016 Fixed dateflag in case dates input as m/d/year
02 Mar 2016 added use command and noisily command to module specific do files
15 Mar 2016 Created an unedited HHQCombined dataset that keeps original merged HHQ and HHQ_member data
18 Mar 2016 Added code to export all completed HHQ and FRS interviews less than 10 minutes and to total
	by RE any interviews less than 10 minutes
1May2016 Adapted to include changes made to forms before Rajasthan Round 1 and Nigeria Round 3 went into field
17 June 2016 Included age, marital status, and whether a current user of contraception for FQ int less 10
20 July 2016 Updating to include changes made to forms beginning July 2016/GHana Round 5
12 Aug 2016 Updated commands counting number of EAs with information to make sure //
counting unique geographic locations

v19-LZ-2017.03.31 link was blank for unlinked forms and being dropped as duplicate.  Link replacement coded
v20 - lz- 2017.05.09 added 4th module and added .do to HHQdo local
*v21 - lz-2017.11.15 fixed issues with duplicate link in FRQ.  Duplicates showing up even though they are fixed
*/
