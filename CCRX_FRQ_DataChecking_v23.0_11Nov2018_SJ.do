...
****** Revision History ********
*September 1, 2015 - revised to allow for additional version of the form
*October 12, 2015 - added fp_ever_user label and encoded
*January 6, 2016 - added sterilization permanent inform to encode as yes/no chunk
*March 1, 2016 - renamed heard_gel to heard_foamjelly
*April 11, 2016 - Beginning with Rajasthan Round 1 (March 2016) forms were programmed
*with some new logic that requires renaming, recoding and dropping additional variables
*v7 17 Jun2016 fixed FQstart so that the duration is correct
*v10 26 Aug 2016 Updated with changes to core questionnaire implemented in July 2016
*v12 26 Octo 2016 Updated age_at_first_use_children and fp_ever_user to correct skipped 
*v15 7 Feb 2017 Added age_first_birth var
	missing values
*v16-SJ-1May2017 updated due to ODK name change
*V17-LZ-19Jun2017 Updated v17 to correct for new DNK option in month and various other changes
*v18-LZ-30Jun2017 Updated to correct that first birth and penult birth were not being correctly replaced
*v19-LZ-1Aug2017 Updated twins skip pattern for first birth
*v20-BL-01Nov2017 Updates based on questionnaire review: added and droped variables, converted stop_using_why 
*				  to select multiple, added follow-up consent, changed denomenator for partner_overall and 
*				  why_not_decision, updated stop_using variable
*v23-SJ-11Nov2018 fix labeling issue of day week month variables
********************************/
...
*REVISION v16.0 former variable stop_using now named stop_using_m_y 
*REVISION: BL 01Nov2017 above revision no longer valid
*capture rename stop_using_m_y stop_using 
...
*REVISION-v17.0 15Jun2017
*REVISION: BL 01Nov2017 stop_using now SPUstop_using
...
*REVISION-v17.0 REFORMAT THE DATE VARIABLES THAT ARE REPLACED WITH PREVIOUS VERSION FORMATTING
...
//REVISION: BL 01Nov2017 pb cd dropped
...
*rename pb_y penultimate_birthv2
*rename cd_y child_deathv2
...
//REVISION: BL 01Nove2017 husband_cohabit_now penultimate_birth child_alive child_death
*			knows_fp_sdp fees_12months fees_12months_amount (replaced with method_fees) san_facility lam_probe dropped
*			
label var	EA					"EA"
...
*label var	husband_cohabit_now					"Partner living with you or staying elsewhere"

//REVISION: BL 01Nov2017 birth_events now individual question (previously calculated from multiple questions)
rename birth_events birth_events_rw
capture label var	birth_events_rw					"How many times have you given birth"
*capture label var	birth_events_live_prompt			
*capture label var	birth_events_check				"All live births?"
*capture label var	birth_events_live				"Confirm all live births"
...
*label var	penultimate_birth				"When did you give birth before the most recent birth?"
*label var	child_alive						"Is your last baby still alive?"
*label var	child_death						"When did your last baby die?"
...
*label var	current_method_check						"Using method"
...
*label var	knows_fp_sdp						"Do you know a place you can obtain a method of FP"
...
*label var	fees_12months				"Have you paid any fees for FP last 12 months"
*label var	fees_12months_amount			"How much did you pay?"
...
//REVISION: BL 01Nov2017 new variables on method switching & partner knowledge
...
*label var					"Have sanitation facility"
...
*label var	version				"FRQ version"

*label var 	lam_probe		"Breastfeeding to avoid pregnancy?"
...
*destring fees_12months_amount, replace
...
//REVISION: BL 01Nov2017 Updated based on dropped and added variables
*						 other_wives & sterilization_permanent_inform now country speific
...
//REVISION: BL 01Nov2017 dropped parnter_living_list
*label define partner_living_list 1 with_respondent 2 elsewhere -99 "-99"
*	encode husband_cohabit_now, gen(husband_cohabit_nowv2) lab(partner_living_list)

//REVISION: BL 03Nov2017 new dwmy_list defined in HHQ DataChecking

//REVISION: SJ 11Nov2017 code dwmy_list
capture label drop dwmy_list
label define dwmy_list 1 "days" 2 "weeks" 3 "months" 4 "years" -99 "-99"

encode menstrual_period, gen(menstrual_periodv2) lab(dwmy_list)
...
/*REVISION: BL 01Nov2017 stop_using_why now select multiple	
label define whystop_list 1 infrequent 2 pregnant 3 wanted_pregnant 4 husband 5 more_effective 6 no_method_available ///
	7 health_concerns 8 side_effects 9 no_access 10 cost 11 inconvenient 12 fatalistic 13 difficult_to_conceive ///
	14 interferes_with_body 15 other -88 "-88" -99 "-99"
	
	encode stop_using_why, gen(stop_using_whyv2) lab(whystop_list)
*/
...
label define whynot_list 1 not_married 2 infrequent_sex 3 menopausal_hysterectomy 4 infecund 5 not_menstruated ///
	...
	*encode why_not_using, gen(why_not_usingv2) lab(whynot_list)
	
label define decision_list 1 you_alone 2 provider 3 partner 4 you_and_provider 5 you_and_partner 6 other -99 "-99"
...
//REVISION - BL 03Nov2017 new label days weeks months years defined in HHQ Data Checking
encode last_time_sex, gen(last_time_sexv2) lab(dwmy_list)

encode pp_method_units, gen(pp_method_unitsv2) lab(dwmy_list)
...
//REVISION: BL 01Nov2017 updated based on 2017 questionnaire review

capture confirm var ever_birth
if _rc==0 {
label var ever_birth "Ever given birth"

*label var children_born "Number of children ever born"
*destring children_born, replace
*label var children_living "Any children living with you"
*label var sons_living	"Number of sons living with you"
*label var daughters_living "Number of daughters living with you"
*destring sons_living, replace
*destring daughters_living, replace

*label var children_away "Any children living away"
*label var sons_away "Number of sons living away"
*destring sons_away, replace
*label var daughters_away "Number of daughters living away"
*destring daughters_away, replace

*label var children_died "Any children who have died"
*label var boys_died "Number of boys who have died"
*destring boys_died, replace
*label var girls_died "Number of girls who have died"
*destring girls_died, replace
*drop total_children_check
...
*label var rhythm_learn "Where did you learn rhythm"
*label var lam_learn "Where did you learn LAM"
*label var rhythm_obtain "Was rhythm the method you wanted"
*label var lam_obtain "Was LAM the method you wanted"
...
label define partner_overall_list 1 "Respondent" 2 "Husband" 3 "Joint" 4 "Other" -99 "-99"
...
*Generate date variable that is only month, day, hour, min, sec, year
*gen double dateup=clock(submissiondate, "MDYhms")
*format dateup %tc
...
*replace startSIF=manual_dateSIF if system_date_check==0
format startSIF %tc
...
*REVISION v18 -30Jun2017 - fixing the first birth/recent birth skip patterns
capture replace first_birth=recent_birth if children_born==1
capture replace first_birth=recent_birth if birth_events==1
*capture replace penultimate_birth=first_birth if birth_events==2
*capture replace penultimate_birth=first_birth if children_born==2 & (recent_birth==first_birth)
*REVISION v19-1Aug2017 fixing skip patterns for twins
...
//REVISION: BL 01Nov2017 dropped penultimate_birth child_death
/*
gen double penultimate_birthSIF=clock(penultimate_birth, "MDY")
format penultimate_birthSIF %tc
replace penultimate_birthSIF=. if regexm(penultimate_birth, "2020")

gen double child_deathSIF=clock(child_death, "MDY")
format child_deathSIF %tc
replace child_deathSIF=. if regexm(child_death, "2020")

gen double begin_usingSIF=clock(begin_using, "MDY")
format begin_usingSIF %tc
replace begin_usingSIF=. if regexm(begin_using, "2020")
*/
...
*gen washing=0 if FRS_result==1
*gen other_modern=0 if FRS_result==1
...
*replace other_modern=1 if current_method_temp`y'=="other_modern" & FRS_result==1
...
gen wn`var'=0 if why_not_using!="" & why_not_using!="-99" & why_not_using!="-88"
forval y=1/`x' {
replace wn`var'=1 if why_not_using_`y'=="`var'"
}
}
//REVISION: SJ get rid of intermediates
drop why_not_using_*
...
//REVISION SJ 14NOV2017 order why not using vars
